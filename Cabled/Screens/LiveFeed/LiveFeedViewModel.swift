        ...
        Task {
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    // Updated: sort carriers by carrier.number (low to high)
                    self.carriers = fetchedCarriers.sorted { $0.number < $1.number }
    ...
    private func handleCarrier(from message: WebSocketMessage) {
        ...
        do {
            ...
            if let index = self.carriers.firstIndex(where: { $0.id == updatedCarrier.id }) {
                ...
                self.carriers[index] = updatedCarrier
                // Updated: re-sort carriers after updating one
                self.carriers.sort { $0.number < $1.number }
