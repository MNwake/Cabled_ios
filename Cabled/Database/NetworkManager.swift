...
class NetworkManager {
    ...
    func fetchRiders() async throws -> [Rider] {
        ...
        do {
            ...
            if let jsonStr = String(data: data, encoding: .utf8) {
                print("Received JSON: from Fetch Riders")
            }
            ...
            decoder.dateDecodingStrategy = .custom { decoder -> Date in
                ...
                if let date = DateFormatter.yyyyMMddTHHmmss.date(from: dateString) {
                    ...
                    print("Fetch Riders: Invalid date format: \(dateString)")
            ...
            print("Decoding error on fetch riders: \(error)")
    ...
    func fetchContestCarriers() async throws -> [ContestCarrier] {
        ...
        do {
            ...
            if let jsonStr = String(data: data, encoding: .utf8) {
                print("Received raw JSON for Contest Carriers: \(jsonStr)")
            } else {
                print("Unable to convert data to string")
            }
            
            // Also, try to convert the data into a JSON object (dictionary or array)
            do {
                let jsonObject = try JSONSerialization.jsonObject(with: data, options: [])
                print("Parsed JSON object: \(jsonObject)")
            } catch {
                print("Error parsing JSON with JSONSerialization: \(error)")
            }
            
            let decoder = JSONDecoder()
            decoder.dateDecodingStrategy = .formatted(DateFormatter.yyyyMMddTHHmmss)
            
            // If the server sends an array at the top level, decode it directly as [ContestCarrier]
            let carriers = try decoder.decode([ContestCarrier].self, from: data)
            print("Decoded Contest Carriers successfully: \(carriers)")
            return carriers
        } catch {
            print("Decoding error on Contest Carriers: \(error)")
            throw RequestError.unableToComplete
        }
    }
    ...
    func fetchRiderStats(cursor: String? = nil,
        ...
        guard let url = components?.url else {
            print("Invalid URL: '\(String(describing: components))")
            throw RequestError.invalidURL
        }
    ...
    func fetchParks() async throws -> [Park] {
        ...
        do {
            ...
            print("Successfully decoded Parks: \(parks)")
            return parks
        } catch {
            print("fetchPark network manager error: \(error)")
            throw RequestError.unableToComplete
        }
    }
    
    func fetchScorecards(forRiders riderIds: [String]? = nil, cursor: String? = nil) async throws -> [Scorecard] {
        var components = URLComponents(string: scorecardURL)
        ...
        if let url = components?.url {
            print("URL with query parameters: \(url)")
        } else {
        ...
        do {
            ...
            if let jsonStr = String(data: data, encoding: .utf8) {
                ...
                print("Failed to convert received data into a string")
            }
            ...
            let scorecardsResponse = try decoder.decode(Scorecards.self, from: data)
            print("Decoded Scorecards: \(scorecardsResponse)")
            return scorecardsResponse.data
        } catch {
            print("Error fetching or decoding JSON Scorecards: \(error)")
            throw RequestError.unableToComplete
        }
    }
    ...
    func fetchRiderProfile(riderID: String) async throws -> RiderProfile {
        ...
        do {
            ...
            print("Error fetching or decoding JSON Rider Profile: \(error)")
            throw RequestError.unableToComplete
        }
    }
